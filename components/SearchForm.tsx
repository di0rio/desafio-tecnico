  ...
  return (
    <form onSubmit={handleSubmit} className="flex gap-3 max-w-xl">
      <div className="relative flex-1">
        <Search size={16} className="absolute left-4 top-1/2 -translate-y-1/2 pointer-events-none" style={{ color: "var(--color-muted)" }} />
        <input type="search" value={query} onChange={(e) => setQuery(e.target.value)}
          placeholder="Buscar por título, conteúdo ou tag..." autoFocus
        ...
        {query && (
          <button type="button" onClick={() => { setQuery(""); startTransition(() => { router.push("/search"); }); }}
            className="absolute right-3.5 top-1/2 -translate-y-1/2" style={{ color: "var(--color-muted)" }}>
            <X size={15} />
          </button>
        )}
      </div>
      <button type="submit" disabled={!query.trim() || isPending}
        ...
        {isPending ? <Loader2 size={15} className="animate-spin" /> : <Search size={15} />} Buscar
      </button>
    </form>
  );
}